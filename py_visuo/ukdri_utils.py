...
from py_visuo import visualise
from py_visuo import custom_colors


def create_activity_plot(df, patient_list, month_list, year, location_list, column_patient='patient_id', column_date='date',
    ...
    for patient in patient_list:
        for month in month_list:
            ...
            for location in location_list:
                ...
                current_df = current_df[current_df[column_date].dt.month.isin(month)].reset_index(drop=True)
                current_df = current_df[current_df[column_date].dt.year==year].reset_index(drop=True)

                current_df['date_more'] = current_df[column_date].astype(
                    str).str.split().str[0]
                current_df['time_of_day'] = pd.to_datetime(
                    '2000-01-01 ' + current_df[column_date].dt.time.astype(str))
...
def test():
    ...
    create_activity_plot(activity_df, patient_list, month_list, year, location_list, column_patient='patient_id', column_date='date',
